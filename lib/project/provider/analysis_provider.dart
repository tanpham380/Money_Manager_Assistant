...
class AnalysisProvider with ChangeNotifier {
    ...
    return transactions.where((transaction) {
      ...
      try {
        ...
        switch (_selectedDateOption) {
          case 'Today':
            return transactionDate.isAfter(todayDT.subtract(Duration(days: 1))) &&
                   transactionDate.isBefore(todayDT.add(Duration(days: 1)));
                   
          case 'This week':
            return transactionDate.isAfter(startOfThisWeek.subtract(Duration(days: 1))) &&
                   transactionDate.isBefore(startOfThisWeek.add(Duration(days: 7)));
                   
          case 'This month':
            return transactionDate.isAfter(startOfThisMonth.subtract(Duration(days: 1))) &&
                   transactionDate.isBefore(DateTime(todayDT.year, todayDT.month + 1, 1));
                   
          case 'This quarter':
            return transactionDate.isAfter(startOfThisQuarter.subtract(Duration(days: 1))) &&
                   transactionDate.isBefore(DateTime(startOfThisQuarter.year, 
                       startOfThisQuarter.month + 3, 1));
                       
          case 'This year':
            return transactionDate.isAfter(startOfThisYear.subtract(Duration(days: 1))) &&
  ...
  List<CategorySummary> _mapToSummaryList(
    ...
    // Tính tổng
    final total = categoryMap.values.fold<double>(0.0, (sum, value) => sum + value);
    ...
    final List<CategorySummary> result = majorCategories.mapIndexed((index, entry) {
